...
(function($){
	...
	_initNode = function(setting, level, n, parentNode, isFirstNode, isLastNode, openFlag) {
		...
		if (n[childKey] && n[childKey].length > 0) {
			...
			//add by xychun 2013/04/26
			n.isParent = (n.isParent==undefined) ? setting.data.keep.isParentDefaultValue : n.isParent;
	...
	event = {
		...
		bindTree: function(setting) {
			...
			o.bind('selectstart', function(e){
					...
					var n = e.srcElement? e.srcElement.nodeName.toLowerCase():e.originalEvent.srcElement.nodeName.toLowerCase();
			...
			$.ajax({
				...
				success: function(msg) {
					...
					if (node) {
						node.isAjaxing = null;
						node.zAsync = true;
						if ($.isArray(newNodes) && newNodes.length ==0){
							//add by xychun 2013-04-26 如果没有子节点，设置当前节点为子节点
							node.isParent = false;
						}
					}
